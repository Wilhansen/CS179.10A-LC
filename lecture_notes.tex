...
\subsection{Applications (2)}
	...
	Given two objects, $A$, $B$, with positions, $p_A$, $p_B$, and velocities $v_A$, $v_B$, how do you determine if the two objects are approaching or separating from each other? Note that $p_B - p_A$ is the relative position of $B$ with respect to $A$
	$$
